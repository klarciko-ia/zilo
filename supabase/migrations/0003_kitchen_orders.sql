-- Kitchen display (KDS) tickets, independent rows for realtime UI.
...
create index if not exists idx_kitchen_orders_restaurant on kitchen_orders(restaurant_id);
...
-- Resolve menu_item_id only when UUID is valid and exists in menu_items (else NULL).
...
-- Atomic: kitchen ticket + merge into open table bill (or create bill).
...
-- Supabase Realtime: ignore error if already in publication (re-run migration).
